...
#' Read global land fraction data
#'
#' Read globally gridded time independent data from a netCDF file.
#'
#' @param filename File name to read from
#' @param len Maximum length to read
#' @param tag Identifier for the source of this dataset
#' @param varname Name of the variable to read
#' @export
read.area <- function(filename, tag=basename(filename), varname='sftlf',
                         latvar='lat', lonvar='lon')
{
    ...
    assertthat::assert_that(all(dim(v3d) == c(nlon, nlat)))

    ## reorganize and flatten the 3-D array into a 2d array of ntime x ngrid
    ## As we do this, we will also make latitude the most rapidly varying index
    ## for the individual time slices.

    variable <- aperm(v3d, c(2,1))
    dim(variable) <- c(1, nlat*nlon)
    ...
    gd <- list(vardata=as.matrix(variable), globalop=as.matrix(areafac), lat=lat, lon=lon, time = 1,
               tags=tagout)
    class(gd) <- 'griddata'
    gd
}
