---
title: "Timing Land vs Water Fitting"
...
This is an experiment under EPA-ALPs to try and create a model from MIROC-5 data such that we can take Hector data and feed it into the model and get an approximated downscaled temperature without the cost of running a full physical model like MIROC-5. 

Currently attempting to fit the model leads to prohibitively high computation times, and so this experiment aims to determine if spliting land and sea cells apart (and grouping the sea cells) will make the calculation time decrease. 

This document was (re)constructed on `r Sys.Date()`
...
```
## Allocate land only cells, ocean only cells, and coastal cells
...
Now we do a fit for every batch size, but that prints too much info so it is silenced.
...
A graph of the efficiency of each run. 
```{r, echo = FALSE}
